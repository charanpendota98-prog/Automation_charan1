...
def clean_public_article(html: str) -> str:
    ...
    for pattern in (
        ...
        # Older drafts may lack the generated class but still expose the
        # machine-facing badge text; remove that chrome as well.
        r'<p\b[^>]*>.*?(?:Reading Time|reading time|చదవడానికి సమయం).*?</p>',
