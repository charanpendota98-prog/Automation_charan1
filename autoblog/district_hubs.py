...
def build_hub_html(district: str, state: str, posts: List[Dict],
    ...
    rows = "".join(
        '<tr><td><a href="{lk}">{ti}</a></td><td>{dt}</td></tr>'.format(
            lk=seo.safe_url(p.get("link", "")), ti=esc((p.get("title") or "")[:70]),
...
def rebuild(state: str = "", apply: bool = False,
    ...
    for row in rows:
        row.pop("_posts", None)
    # v100: kotha/update ayina hub pages ni INSTANT INDEXING ki pampali.
    # Ippati varaku posts matrame submit ayyevi (pipeline._after_publish_push);
    # hub/district pages organic crawl kosam wait chesevi — ante konni rojulu.
    # Best-effort: key ledu / network ledu ⇒ silent skip (rebuild aagadu).
    if apply:
        ...
        if links:
            try:
                ...
                if indexnow.submit(links):
                    log.info("District hubs: %d URLs IndexNow ki pampam",
                             len(links))
            except Exception as exc:  # noqa: BLE001 — advisory
                log.debug("district hub indexnow skip: %s", exc)
