...
def _norm_url(url: str, site: str = "") -> str:
    """Compare cheyyadaniki URL normalise (scheme/host/trailing slash/query).

    v100 BUG FIX: RELATIVE links (`/tspsc-group-2/`) resolve avvatledu — avi
    "studentup.in/..." ga kaakunda "/tspsc-group-2" ga migilipoyi graph lo
    match avvevi kaavu. Result: nijamga inbound links unna posts kuda
    **orphans ga report** ayyevi (false positive) → anavasaram ga inka links
    add ayyevi. WordPress themes chala chota relative links emit chestayi
    kabatti idi real-world lo chala common.
    """
    u = (url or "").strip()
    u = re.sub(r"[?#].*$", "", u)
    if u.startswith("//"):                       # protocol-relative
        u = "https:" + u
    if u.startswith("/"):                        # site-relative → absolute
        host = re.sub(r"^https?://", "", (site or getattr(config, "WP_SITE", "")))
        host = re.sub(r"^www\.", "", host).rstrip("/")
        u = f"{host}{u}" if host else u
    u = re.sub(r"^https?://", "", u)
    u = re.sub(r"^www\.", "", u)
...
def _is_internal(url: str, site: str) -> bool:
    n, s = _norm_url(url, site), _norm_url(site)
    if not n:
        return False
    if n.startswith("/"):
        return True
...
def insert_link(html: str, url: str, anchor: str) -> Optional[str]:
    ...
    # v100: scheme guard — `javascript:`/`data:` URL eppudu href lo raakudadu
    # (WP sanitiser save cheyyochu, kaani manam emit chesedi safe ga undali).
    href = seo.safe_url(url)
    if not href:
        log.warning("link insert skip — unsafe URL: %r", url[:60])
        return None
    ...
    link = f'<a href="{href}">{_html.escape(anchor)}</a>'
