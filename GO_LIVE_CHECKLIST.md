...
| Test suites (v105) | **85/85** pass (`python run.py --test-all`) · saved-engine **53/53** |
...
| Discover + CWV (v94) | 1200px Discover image · og:image dims · Article schema `image` · img width/height (CLS) · touch-action (INP) — `python tests/v94_test.py` |
| In-body signals (v95) | content-lopala image (`.su-figure`, CLS-safe, lazy) · in-body contextual links (`.su-ctx`) · URL ≤75 + img-alt parity · pin gate **68/68** — `python tests/v95_test.py` |
| Menu + icons (v93) | grouped dropdown menu (Jobs ▾ / More ▾) + brand SVG icons verified — theme `inc/template.php` fallback + `style.css`; proof: `python tests/v93_test.py` |
| Saved / bookmarks (v92) | readers 🔖 save-for-later — theme `inc/saved.php` + `assets/js/studentup-saved.js` (localStorage · DB/cookie ledu) · `[studentup_saved]` shortcode → /saved/ page · proof: `python tests/v92_test.py` |
...
- [ ] **2. WordPress setup** — Rank Math; theme install (**v74**: theme **1.7.2** —
      ...
      **v98 zip:** 50 files · 694 KB · theme **1.9.8** · sha256
      `4215621aef543934bed37143fa7c4fd7769859404ef0e06d5fcf19f25714389d`
...
*Last updated: v105 (2026-09-22) · 85/85 suites · pin gate 100/100 · AdSense readiness 97% · 53/53 saved-engine · 164/164 runtime · theme v1.9.8 · v105 rollback safety + provenance/value + live originality*
