...
def test_theme_integration() -> None:
    ...
    assert re.search(r"STUDENTUP_VERSION',\s*'([^']+)'", fn).group(1) == "1.9.5"
...
def test_zip_packaged() -> None:
    ...
    assert re.search(r"Version:\s*1\.9\.5", css), "zip lo css version 1.9.5 kaadu"
    php = z.read("studentup/functions.php").decode("utf-8")
    assert re.search(r"STUDENTUP_VERSION',\s*'1\.9\.5'", php), "zip lo php version 1.9.5 kaadu"
