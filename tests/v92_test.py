...
def test_theme_integration() -> None:
    ...
    assert re.search(r"STUDENTUP_VERSION',\s*'([^']+)'", fn).group(1) == "1.9.7"
...
def test_version_parity_193() -> None:
    ...
    for entry in ("= 1.9.0", "= 1.9.1", "= 1.9.2", "= 1.9.3", "= 1.9.4", "= 1.9.5", "= 1.9.6", "= 1.9.7"):
...
def test_zip_packaged() -> None:
    ...
    assert re.search(r"Version:\s*1\.9\.7", css), "zip lo css version 1.9.7 kaadu"
    php = z.read("studentup/functions.php").decode("utf-8")
    assert re.search(r"STUDENTUP_VERSION',\s*'1\.9\.7'", php), "zip lo php version 1.9.7 kaadu"
