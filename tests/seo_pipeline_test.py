...
def test_full_pipeline(url, wp_base):
    ...
    # auto-classifier: SSC recruitment title -> Central Govt Jobs (live-site name)
    assert pl.classify_category(
        "SSC New Recruitment 2026 Notification Released"
    ) == "Central Govt Jobs", pl.classify_category("SSC New Recruitment 2026")
    assert pl.classify_category("NSP Scholarship Apply Online 2026") == "Scholarships"
    assert pl.classify_category("Random Education Topic") == "Online Education"
