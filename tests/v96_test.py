...
def test_theme_version_and_zip() -> None:
    ...
    assert css == php == stable == "1.9.7", f"parity tappu: {css}·{php}·{stable}"
    ...
    assert "Version: 1.9.7" in style, "zip stale (build_wp_theme.py run cheyandi)"
    assert ".su-upnext" in style and ".su-join-strip" in style
    print(f"      theme 1.9.7 · zip {len(names)} files · upnext + CSS inside ✔")
