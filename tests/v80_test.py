...
Checks (offline only):
  ...
  * php parse new files
  * docs: README v80 + MANUAL PART 39 + 66/66
...
def test_docs_v80():
    ...
    assert "### v80" in readme and "66/66" in readme
    assert "PART 39" in manual and "v80" in manual and "66/66" in manual
    assert "66/66" in go_live
    for name, txt in (("README", readme), ("MANUAL", manual),
                      ("GO_LIVE", go_live)):
        assert "164/164" in txt, f"{name} lo jsdom claim poyindi"
    print("  docs: README v80 + MANUAL PART 39 + 66/66 ✔")
...
TESTS = [
    ...
    ("docs: v80 + PART 39 + 66/66", test_docs_v80),
]
