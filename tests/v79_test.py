...
Checks (offline only):
  ...
  * docs: README v79 + MANUAL PART 38 + 61/61
...
def test_miss_audit_surface():
    ...
    print(f"  miss-audit: single surface 13/13 + join x2 ✔")
...
def test_docs_v79():
    ...
    assert suites == 61, f"suites {suites} (v79 tho 61 expect)"
    readme = read(ROOT / "README.md")
    manual = read(ROOT / "MANUAL_ADVANCED_CHECKLIST.md")
    go_live = read(ROOT / "GO_LIVE_CHECKLIST.md")
    assert "### v79" in readme and "61/61" in readme
    assert "PART 38" in manual and "v79" in manual and "61/61" in manual
    assert "61/61" in go_live
    for name, txt in (("README", readme), ("MANUAL", manual),
                      ("GO_LIVE", go_live)):
        assert "164/164" in txt, f"{name} lo jsdom claim poyindi"
    print("  docs: README v79 + MANUAL PART 38 + 61/61 ✔")
...
TESTS = [
    ...
    ("docs: v79 + PART 38 + 61/61", test_docs_v79),
]
